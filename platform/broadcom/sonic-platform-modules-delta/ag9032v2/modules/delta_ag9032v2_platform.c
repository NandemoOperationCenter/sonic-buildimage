...
static ssize_t for_status(struct device *dev, struct device_attribute *dev_attr, char *buf){
    ...
    switch (attr->index) {
        case SFP_IS_PRESENT:
            ...
            return scnprintf(buf, PAGE_SIZE, "0x%x%02x\n", data, ret_sfp);
        ...
        default:
            mutex_unlock(&dni_lock);
            return scnprintf(buf, PAGE_SIZE, "%d not found", attr->index);
    }
}
...
static ssize_t get_cpld_reg(struct device *dev, struct device_attribute *dev_attr, char *buf) 
{
    ...
    switch (mask) 
    {
        case 0xff:
            mutex_unlock(&dni_lock);
            return scnprintf(buf, PAGE_SIZE, "0x%02x%s", value, note);
        case 0x0f:
        case 0x03:
            mutex_unlock(&dni_lock);
            return scnprintf(buf, PAGE_SIZE, "0x%01x%s", value, note);
        case 0x0c:
            mutex_unlock(&dni_lock);
            value = value >> 2;
            return scnprintf(buf, PAGE_SIZE, "0x%01x%s", value, note);
        ...
        default :
            ...
            mutex_unlock(&dni_lock);
            return scnprintf(buf, PAGE_SIZE, "%d%s", value, note);
    }
}
...
static struct cpld_mux_platform_data ag9032v2_cpld_mux_platform_data[] = {
    {
        .parent         = BUS0,
...
{
    {
        .name           = "delta-ag9032v2-cpld-mux",
        .id             = 0,
        .dev            = {
                .platform_data   = &ag9032v2_cpld_mux_platform_data[0],
                .release         = device_release,
        },
    },
};
...
//    struct i2c_client *client;
    ...
    cpld_pdata = ag9032v2_cpld_platform_data;
